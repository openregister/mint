...
docker run -d --name=mintAppDbFastForward \
    --volume /srv/mint:/srv/mint \
    jstepien/openjdk8 java -Dfile.encoding=UTF-8 -jar /srv/mint/mint.jar db fast-forward /srv/mint/mint-config.yaml --all

docker run -d --name=mintAppDataMigration \
    --volume /srv/mint:/srv/mint \
    jstepien/openjdk8 java -Dfile.encoding=UTF-8 -jar /srv/mint/mint.jar db migrate /srv/mint/mint-config.yaml
